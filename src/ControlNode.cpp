...
#include "KI/KIRecoverVision.h"
#include "KI/KITakeOff.h"
...
ControlNode::ControlNode()
{
    ...
    noCommandsChannel = nh_.resolveName("tum_ardrone/nocommands");
    manualScaleChannel = nh_.resolveName("tum_ardrone/manualScaleInterval");
    manualKeyframeChannel = nh_.resolveName("/lsd_slam/manualKeyframe");
	...
	noCommandsPublisher = nh_.advertise<std_msgs::Empty>(noCommandsChannel, 1);
	manualScalePublisher = nh_.advertise<std_msgs::Bool>(manualScaleChannel, 1);
	manualKeyframePublisher = nh_.advertise<std_msgs::Empty>(manualKeyframeChannel, 1);
	...
	lagFile.open("Flight 1/lagtimings.txt", std::fstream::out | std::fstream::trunc);
}
...
// Erste Time statt 33 auf 29 gesetzt
double lagTimes[] = {29, 58, 77, 95, 95, 95, 95, 95, 95};
...
void ControlNode::droneposeCb(const tum_ardrone::filter_stateConstPtr statePtr)
{
	...
	while(!recovering && !goBackKI && currentKI == NULL && commandQueue.size() > 0)
		popNextCommand(statePtr);

	// if there is no current KI now, we obviously have no current goal -> send drone hover
	if(currentKI != NULL || (isControlling && autonomousDiscovery && sentNoCommandMessage))
	{
		// Ptam good or very good
		if(statePtr->ptamState == 3 || statePtr->ptamState == 4 || true) // CHANGED...
		{
			if(compensateLag && !recovering && ros::Time::now() > goBackTime && false)
			{
				if(!goBackKI)
				{
					ROS_INFO("Going back to origin, because Lag is going to happen soon...");
					goBackKI = new KIFlyTo(
									DronePosition(
									/*TooN::makeVector(statePtr->x,statePtr->y,statePtr->z),*/TooN::makeVector(0, 0, 0) + parameter_referenceZero.pos,
									parameter_referenceZero.yaw/*statePtr->yaw*/), // keeping the yaw!
									100,
									parameter_MaxControl,
									0.3,
									0.3
									);
					goBackKI->setPointers(this,&controller);
					manualKeyframePublisher.publish(std_msgs::Empty());
				}
				if(currentKI != 0)
				{
					goBackKI->update(statePtr);
				}else
				{
			...
			else
			{
				...
				if(recovering)
				{
					if(!didLagHappenedProd)
					{
						ROS_WARN("PTAM instantly recovered vision...");
			...
			{
				numLags++;
				lastLagTime = ros::Time::now();
				compensateLag = true;
				ros::Duration adding(lagTimes[numLags-1]- 3.0);
				ROS_INFO("Adding %f to the time", adding.toSec());
				goBackTime = lastLagTime + adding;
				ROS_INFO("Lag happened.");
				if(goBackKI)
				{
					delete goBackKI;
					goBackKI = 0;
				}
				sentNoCommandMessage = false;
			}
			visionHadOnce = true;
		}else
		{
			...
			{
				ROS_INFO("New Lag: Time since last lag: %f", (ros::Time::now() - lastLagTime).toSec());
				lagFile << (ros::Time::now() - lastLagTime).toSec() << std::endl;

				ROS_DEBUG("Vision lost -> sending hover");
				sendControlToDrone(hoverCommand);
				ROS_INFO("Adding recover KI");
				KIRecoverVision *newKI = new KIRecoverVision(currentKI);
				newKI->setPointers(this, &controller);
				currentKI = newKI;
				recovering = true;
				didLagHappenedProd = false;
			}

			if(statePtr->ptamState != KIRecoverVision::PTAM_IDLE && !didLagHappenedProd)
			{
				didLagHappenedProd = true;
				if(visionHadOnce)
				{
					...
					lastLagTime = ros::Time::now();
					compensateLag = true;
					ros::Duration adding(lagTimes[numLags-1]- 4.0);
					ROS_INFO("Adding %f to the time", adding.toSec());
					goBackTime = lastLagTime + adding;
					ROS_INFO("Lag happened.");
					if(goBackKI)
	...
	{
		if(autonomousDiscovery && !sentNoCommandMessage)
		{
			ROS_INFO("Sending noCommand-message");
			noCommandsPublisher.publish(std_msgs::Empty());
			sentNoCommandMessage = true;
		}
...
void ControlNode::popNextCommand(const tum_ardrone::filter_stateConstPtr statePtr)
{
	...
	while(currentKI == NULL && commandQueue.size() > 0)
	{
		...
		else if(command == "newtakeoff")
		{
			currentKI = new KITakeOff();
			currentKI->setPointers(this, &controller);
			commandUnderstood = true;
		}
		...
		else if(sscanf(command.c_str(),"gotoraw %f %f %f %f",&parameters[0], &parameters[1], &parameters[2], &parameters[3]) == 4)
		{
			currentKI = new KIFlyTo(
				DronePosition(
		...
		else if(sscanf(command.c_str(),"gotohov %f %f %f %f",&parameters[0], &parameters[1], &parameters[2], &parameters[3]) == 4)
		{
			currentKI = new KIFlyTo(
				DronePosition(
				TooN::makeVector(parameters[0],parameters[1],parameters[2]) + parameter_referenceZero.pos,
		...
		}else if(sscanf(command.c_str(), "discover %f", &parameters[0]) == 1)
		{
			autonomousDiscovery = parameters[0] == 1;
			commandUnderstood = true;
		}
...
void ControlNode::updateControl(const tum_ardrone::filter_stateConstPtr statePtr) {
	// TODO (lsd): Maybe this is not good (to comment this part out)...
	if (currentKI->update(statePtr) && (commandQueue.size() > 0 || autonomousDiscovery)) {
...
void ControlNode::manualScaleActivate(bool start)
{
	std_msgs::Bool msg;
	msg.data = start;
	manualScalePublisher.publish(msg);
}
