...
void calculateCommand(){
    ...
    //    cout <<"(KeyFrameToWorldSE3*LSDSLAM__camToKeyFrame_World).get_translation() "<< endl<<(KeyFrameToWorldSE3*LSDSLAM__camToKeyFrame_World).get_translation() <<endl;
        ...
        cout <<"Difference is (x,y,z,yaw): "<<endl<<result[0]-x_kf << "," <<result[1]-y_kf << " , "<<result[2]-z_kf <<" , "<<  testYaw - yaw_kf<< endl; //For chec
}
public:
...
void run(){
    cin >> RecTime;
    ros::Duration recordTime = ros::Duration(RecTime);    //record for 3 minutes
    ros::Rate r(10); // 10 hz
